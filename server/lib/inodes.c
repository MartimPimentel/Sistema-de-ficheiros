...
/*
 * Checks if the inode file is opened or not
 * Input:
 *  - inumber: identifier of the i-node
 * Returns:
 *    flag: 0 if file is closed and 1 if file is opened
 */
int inode_isFileOpen(int inumber){
    int flag;
    lock_inode_table();
    flag = inode_table[inumber].isFileOpen;
    unlock_inode_table();
    return flag;
}

/*
 * Closes the inode file
 * Input:
 *  - inumber: identifier of the i-node
 * Returns:
 *    0:if successful
 *   -8: if File is not open
 *   -11: if any other error occurs
 */
int inode_closeFile(int inumber){
    lock_inode_table();
    ...
    if (inode_table[inumber].isFileOpen){
        inode_table[inumber].isFileOpen = 0;
        unlock_inode_table();
        return 0;
...
/*
 * Opens the inode file
 * Input:
 *  - inumber: identifier of the i-node
 * Returns:
 *    0:if successful
 *   -9: if file is open
 *   -11: if an ohter error occurs
 */
int inode_OpenFile(int inumber){
    lock_inode_table();
    ...
    if (!(inode_table[inumber].isFileOpen)){
        inode_table[inumber].isFileOpen = 1;
        unlock_inode_table();
        return 0;
